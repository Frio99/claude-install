...
> ⚠️ **需要可访问海外服务的网络**：登录和使用 Claude Code 要访问 `claude.ai` / `api.anthropic.com`。中国大陆用户需先开启**网络代理 / 加速工具**并切到「**全局**」模式，否则登录不上、用不了。
...
2. 输入 `claude`，按回车
3. 它会自动弹出浏览器，让你**登录**（确保此时网络代理 / 加速工具已开启全局）
...
**Q：要花钱吗？**
A：这个安装脚本免费。但用 Claude Code 需要你有 Claude 官方的订阅账号（登录时用），并且网络能访问海外服务。
...
**Q：登录打不开 / 一直转圈 / 报网络错误？**
A：检查你的网络代理 / 加速工具是否已开启「全局」模式。Claude 登录和运行都要访问海外服务器，国内直连访问不了。
